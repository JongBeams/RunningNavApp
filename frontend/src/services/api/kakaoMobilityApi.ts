...
/**
 * 카카오 모빌리티 API를 통한 보행자 경로 계산
 */
export const getKakaoRoute = async (
  startLat: number,
  startLng: number,
  goalLat: number,
  goalLng: number,
  ...
  try {
    ...
    console.error('[KakaoMobility] 경로 계산 실패:', error);
 ...
 */
export const getKakaoRouteBetweenPoints = async (
  startLat: number,
  startLng: number,
  goalLat: number,
  goalLng: number,
  ...
  return getKakaoRoute(startLat, startLng, goalLat, goalLng, middlePoints);
};
